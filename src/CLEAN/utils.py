...
# 文件: src/CLEAN/utils.py

import numpy as np
from sklearn.neighbors import NearestNeighbors
import csv

def write_knn_weighted_votes(emb_train, id_ec_train, emb_test, id_ec_test,
                             csv_name, k=10, tau=None):
    """
    基于距离加权 k-NN 投票选 EC:
    ...
    knn.fit(emb_train)
    # 2. 计算所有测试样本的 k 近邻
    dists, idxs = knn.kneighbors(emb_test)  # shapes: (n_test,k)

    # 如果 tau 未指定，就用所有测试-训练对距离的平均
    if tau is None:
        tau = np.mean(dists)

    # 打开输出文件
    with open(f"{csv_name}_knn.csv", 'w', newline='') as f:
        writer = csv.writer(f)
        # 每个测试蛋白
        ...
        for i, test_id in enumerate(id_ec_test.keys()):
            ...
            for dist, idx in zip(dists[i], idxs[i]):
                ...
                for ec in id_ec_train[train_id]:
                    votes[ec] = votes.get(ec, 0.0) + weight

            # 按得票权重排序，取最高的几个 EC
            sorted_ec = sorted(votes.items(), key=lambda x: x[1], reverse=True)
            # 你可以只取 top1，也可以取多个
            top1_ec, top1_score = sorted_ec[0]
            # 写入：蛋白ID, EC:编号/权重(四小数)
            writer.writerow([
                test_id,
                f"EC:{top1_ec}/{top1_score:.4f}"
            ])
