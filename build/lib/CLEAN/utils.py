...
def get_ec_id_dict(csv_name: str) -> dict:
    ...
    for i, rows in enumerate(csvreader):
        if i > 0:
            id_ec[rows[0]] = rows[4].split(';')
            for ec in rows[4].split(';'):
...
def get_ec_id_dict_test(csv_name: str) -> dict:
    csv_file = open(csv_name)
    csvreader = csv.reader(csv_file, delimiter='\t')
    id_ec = {}
    ec_id = {}
...
def load_esm(lookup):
    file_path = './data/esm2_data/' + lookup + '.pt'
    if not os.path.exists(file_path):
        print(f"WARNING: ESM embedding file not found: {file_path}")
        # Return a tensor of correct shape filled with zeros as fallback
        return torch.zeros(1, 2560, device="cpu")
...
def esm_embedding(ec_id_dict, device, dtype):
    ...
    for ec in list(ec_id_dict.keys()):
        ...
        for id in ids_for_query:
            file_path = './data/esm2_data/' + id + '.pt'
            if os.path.exists(file_path):
                local_esm_cat.append(load_esm(id))
            else:
                missing_files += 1
                if missing_files <= 10:  # Limit the number of missing files printed
                    print(f"Missing ESM file: {file_path}")
                elif missing_files == 11:
                    print("Too many missing files, suppressing further warnings...")
                
        if local_esm_cat:  # Only concatenate if we have valid embeddings
            esm_to_cat = torch.cat(local_esm_cat, dim=0)
            esm_emb.append(esm_to_cat)
        
    if not esm_emb:
        raise ValueError("No valid ESM embeddings found. Check if your EC numbers match existing embedding files.")
        
    print(f"Total missing embedding files: {missing_files}")
...
def model_embedding_test(id_ec_test, model, device, dtype):
    ...
    for id in ids_for_query:
        file_path = './data/esm2_data/' + id + '.pt'
        if os.path.exists(file_path):
            esm_to_cat.append(load_esm(id))
        else:
            missing_files += 1
            if missing_files <= 10:
                print(f"Missing test ESM file: {file_path}")
            elif missing_files == 11:
                print("Too many missing test files, suppressing further warnings...")
    
    if not esm_to_cat:
        raise ValueError("No valid test ESM embeddings found. Check if your test IDs match existing embedding files.")
    
    print(f"Total missing test embedding files: {missing_files}")
...
def model_embedding_test_ensemble(id_ec_test, device, dtype):
    ...
    for id in ids_for_query:
        ...
        if os.path.exists(file_path):
            ...
            missing_files += 1
            if missing_files <= 10:
                print(f"Missing ensemble test ESM file: {file_path}")
            elif missing_files == 11:
                print("Too many missing ensemble test files, suppressing further warnings...")
    
    if not esm_to_cat:
        raise ValueError("No valid ensemble test ESM embeddings found. Check if your test IDs match existing embedding files.")
    
    print(f"Total missing ensemble test embedding files: {missing_files}")
    esm_emb = torch.cat(esm_to_cat).to(device=device, dtype=dtype)
    return esm_emb
...
def ensure_dirs():
    paths = ['data/distance_map', 'data/esm2_data', 'data/model', 
...
def mutate_single_seq_ECs(train_file):
    ...
    for id in id_ec.keys():
        for ec in id_ec[id]:
            if ec in single_ec and not os.path.exists('./data/esm2_data/' + id + '_1.pt'):
                single_id.add(id)
                break
